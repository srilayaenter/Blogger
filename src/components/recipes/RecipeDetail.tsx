...
import { RecipePrevNextNav } from "./RecipePrevNextNav";
...
export function RecipeDetail({
  ...
  return (
    <>
            ...
            {sortedIngredients.map((ingredient) => {
              ...
              return (
                <li key={ingredient.id}>
                  {[displayQuantity(ingredient.quantity, locale), unit, name]
                    .filter(Boolean)
                    .join(" ")}
                  {notes ? ` (${notes})` : null}
                </li>
      ...
      <RecipePrevNextNav previous={previous} next={next} locale={locale} />
    </>
  );
}
